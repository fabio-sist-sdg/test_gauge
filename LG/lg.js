...
! function() {
    ...
    t.innerHTML = "\n\t\t<div>\n\t\t\t<svg width=100% height=100% id=pwclfg>\n\t\t\t</svg>\n\t\t</div>\n\t\t<style>\n\t\t\t.liquidFillGaugeText {\n\t\t\t\tfont-family: Arial;\n\t\t\t\tfont-weight: bold;\n\t\t\t}\n\t\t</style>\n\t";
    customElements.define("com-sdg-lg", class extends HTMLElement {
        constructor() {
            super(), this.attachShadow({
                mode: "open"
            }).appendChild(t.content.cloneNode(!0)), this.addEventListener("click", t => {
                t = new Event("onClick"), this.dispatchEvent(t)
            }), this._props = this.liquidFillGaugeDefaultSettings()
        }
        onCustomWidgetBeforeUpdate(t) {}
        onCustomWidgetAfterUpdate(t) {
            "waveColor" in t && (this._props.waveColor = t.waveColor), "circleColor" in t && (this._props.circleColor = t.circleColor), "waveTextColor" in t && (this._props.waveTextColor = t.waveTextColor), "textColor" in t && (this._props.textColor = t.textColor), "opacity" in t && (this.style.opacity = t.opacity), "value" in t && (this.$value = t.value), "maxValue" in t && (this._props.maxValue = t.maxValue), "waveCount" in t && (this._props.waveCount = t.waveCount), "circleThickness" in t && (this._props.circleThickness = t.circleThickness), "waveHeight" in t && (this._props.waveHeight = t.waveHeight), "displayPercent" in t && (this._props.displayPercent = t.displayPercent), "waveAnimate" in t && (this._props.waveAnimate = t.waveAnimate), null != d3.select(this).node().parentNode && null != d3.select(this).node().parentNode && this.drawChart(this.$value, this._props)
        }
        onCustomWidgetResize(t, e) {
            this.$width = t + "px", this.$height = e + "px", this.drawChart(this.$value, this._props)
        }
        connectedCallback() {
            var t = d3.select(this).node();
            this.$width = t.parentNode.parentNode.parentNode.style.width, this.$height = t.parentNode.parentNode.parentNode.style.height, this.drawChart(this.$value, this._props)
        }
        disconnectedCallback() {}
        drawChart(t, e) {
            e.circleThickness = e.circleThickness + "", e.waveCount = e.waveCount + "", e.waveHeight = e.waveHeight + "";
            var a = d3.select(this.shadowRoot).select("div");
            a.style("width", this.$width).style("height", this.$height), a.selectAll("*").remove(), a.append("svg:svg").attr("width", "100%").attr("height", "100%").attr("id", "pwclfg"), this.loadLiquidFillGauge("pwclfg", t, e)
        }
        ...
        loadLiquidFillGauge(t, e, a) {
            ...
            var i, n = d3.select(this.shadowRoot).select("div").select("svg"),
                r = this.$width,
                s = this.$height,
                o = Math.min(parseInt(r), parseInt(s)) / 2,
                l = parseInt(r) / 2 - o,
                d = parseInt(s) / 2 - o,
                c = Math.max(a.minValue, Math.min(a.maxValue, e)) / a.maxValue;
            i = a.waveHeightScaling ? d3.scale.linear().range([0, a.waveHeight, 0]).domain([0, 50, 100]) : d3.scale.linear().range([a.waveHeight, a.waveHeight]).domain([0, 100]);
            var h = a.textSize * o / 2,
                u = parseFloat(e).toFixed(2),
                p = a.valueCountUp ? a.minValue : u,
                v = a.displayPercent ? "%" : "",
                m = a.circleThickness * o,
                w = m + a.circleFillGap * o,
                g = o - w,
                f = g * i(100 * c),
                x = 2 * g / a.waveCount,
                C = 1 + a.waveCount,
            ...
            parseFloat(u) != parseFloat(F(u)) && (F = function(t) {
                return parseFloat(t).toFixed(1)
            }), parseFloat(u) != parseFloat(F(u)) && (F = function(t) {
                return parseFloat(t).toFixed(2)
            });
            for (var y = [], A = 0; A <= 40 * C; A++) y.push({
                x: A / (40 * C),
                y: A / 40
            });
            var M = d3.scale.linear().range([0, 2 * Math.PI]).domain([0, 1]),
                P = d3.scale.linear().range([0, o]).domain([0, o]),
                R = d3.scale.linear().range([0, T]).domain([0, 1]),
                V = d3.scale.linear().range([0, f]).domain([0, 1]),
                H = d3.scale.linear().range([w + 2 * g + f, w - f]).domain([0, 1]),
                _ = d3.scale.linear().range([0, T - 2 * g]).domain([0, 1]),
                $ = d3.scale.linear().range([w + 2 * g, w + .7 * h]).domain([0, 1]),
                k = n.append("g").attr("transform", "translate(" + l + "," + d + ")"),
                I = d3.svg.arc().startAngle(M(0)).endAngle(M(1)).outerRadius(P(o)).innerRadius(P(o - m));
            k.append("path").attr("d", I).style("fill", a.circleColor).attr("transform", "translate(" + o + "," + o + ")");
            var G = k.append("text").text(F(p) + v).attr("class", "liquidFillGaugeText").attr("text-anchor", "middle").attr("font-size", h + "px").style("fill", a.textColor).attr("transform", "translate(" + o + "," + $(a.textVertPosition) + ")"),
                N = d3.svg.area().x(function(t) {
                    return R(t.x)
                }).y0(function(t) {
                    return V(Math.sin(2 * Math.PI * a.waveOffset * -1 + 2 * Math.PI * (1 - a.waveCount) + 2 * t.y * Math.PI))
                }).y1(function(t) {
                    return 2 * g + f
                }),
                S = k.append("defs").append("clipPath").attr("id", "clipWave" + t),
                q = S.append("path").datum(y).attr("d", N).attr("T", 0),
                E = k.append("g").attr("clip-path", "url(#clipWave" + t + ")");
            E.append("circle").attr("cx", o).attr("cy", o).attr("r", g).style("fill", a.waveColor);
            var z = E.append("text").text(F(p) + v).attr("class", "liquidFillGaugeText").attr("text-anchor", "middle").attr("font-size", h + "px").style("fill", a.waveTextColor).attr("transform", "translate(" + o + "," + $(a.textVertPosition) + ")");
            if (a.valueCountUp) {
                var L = function() {
                    var t = d3.interpolate(this.textContent, u);
                    return function(e) {
                        this.textContent = F(t(e)) + v
                    }
                };
                G.transition().duration(a.waveRiseTime).tween("text", L), z.transition().duration(a.waveRiseTime).tween("text", L)
            }
            var U = w + 2 * g - T;

            function W() {
                q.attr("transform", "translate(" + _(q.attr("T")) + ",0)"), q.transition().duration(a.waveAnimateTime * (1 - q.attr("T"))).ease("linear").attr("transform", "translate(" + _(1) + ",0)").attr("T", 1).each("end", function() {
                    q.attr("T", 0), W(a.waveAnimateTime)
                })
            }
            return a.waveRise ? S.attr("transform", "translate(" + U + "," + H(0) + ")").transition().duration(a.waveRiseTime).attr("transform", "translate(" + U + "," + H(c) + ")").each("start", function() {
                q.attr("transform", "translate(1,0)")
            }) : S.attr("transform", "translate(" + U + "," + H(c) + ")"), a.waveAnimate && W(), new function() {
                this.update = function(t) {
                    var e = parseFloat(t).toFixed(2),
                        n = function(t) {
                            return Math.round(t)
                        };
                    parseFloat(e) != parseFloat(n(e)) && (n = function(t) {
                        return parseFloat(t).toFixed(1)
                    }), parseFloat(e) != parseFloat(n(e)) && (n = function(t) {
                        return parseFloat(t).toFixed(2)
                    });
                    var r = function() {
                        var e = d3.interpolate(this.textContent, parseFloat(t).toFixed(2));
                        return function(t) {
                            this.textContent = n(e(t)) + v
                        }
                    };
                    G.transition().duration(a.waveRiseTime).tween("text", r), z.transition().duration(a.waveRiseTime).tween("text", r);
                    var s, o = Math.max(a.minValue, Math.min(a.maxValue, t)) / a.maxValue,
                        l = g * i(100 * o),
                        d = d3.scale.linear().range([w + 2 * g + l, w - l]).domain([0, 1])(o),
                        c = d3.scale.linear().range([0, T]).domain([0, 1]),
                        h = d3.scale.linear().range([0, l]).domain([0, 1]);
                    s = a.waveHeightScaling ? d3.svg.area().x(function(t) {
                        return c(t.x)
                    }).y0(function(t) {
                        return h(Math.sin(2 * Math.PI * a.waveOffset * -1 + 2 * Math.PI * (1 - a.waveCount) + 2 * t.y * Math.PI))
                    }).y1(function(t) {
                        return 2 * g + l
                    }) : N;
                    var u = a.waveAnimate ? _(1) : 0;
                    q.transition().duration(0).transition().duration(a.waveAnimate ? a.waveAnimateTime * (1 - q.attr("T")) : a.waveRiseTime).ease("linear").attr("d", s).attr("transform", "translate(" + u + ",0)").attr("T", "1").each("end", function() {
                        a.waveAnimate && (q.attr("transform", "translate(" + _(0) + ",0)"), W(a.waveAnimateTime))
                    }), S.transition().duration(a.waveRiseTime).attr("transform", "translate(" + U + "," + d + ")")
                }
            }
        }
    })
